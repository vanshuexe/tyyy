...
export const Hero: React.FC<HeroProps> = ({ onOpenConsultation, onExploreEstimator }) => {
  ...
  const handleMouseLeave = () => {
    ...
    const networkNodes = [
      { id: 'net-us', x: 100, y: 40, code: 'us', name: 'USA' },
      { id: 'net-gb', x: 100, y: 240, code: 'gb', name: 'UK' },
      { id: 'net-ie', x: 410, y: 20, code: 'ie', name: 'Ireland' },
      { id: 'net-pt', x: 290, y: 270, code: 'pt', name: 'Portugal' },
      { id: 'net-ch', x: 530, y: 270, code: 'ch', name: 'Swiss' },
      { id: 'net-ae', x: 720, y: 40, code: 'ae', name: 'UAE' },
      { id: 'net-in', x: 720, y: 240, code: 'in', name: 'India' },
    ];

  return (
    <section className="relative overflow-hidden bg-[#fafafa] text-gray-900 pt-20 pb-20 lg:pt-32 lg:pb-32 border-b border-gray-200">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-8 items-center mb-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }} className="flex flex-col items-start text-left">
            {/* Subtle Top Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white shadow-sm border border-gray-200 text-gray-600 text-xs font-semibold tracking-widest uppercase mb-4 transition-transform hover:-translate-y-0.5 duration-300">
            ...
            <div className="flex items-center gap-2 text-[#0b1b36] font-medium text-sm mb-4">
                  ...
                  <foreignObject key={node.id} x={node.x} y={node.y} width="140" height="140" className="overflow-visible">
                    <div className="flex flex-col items-center justify-center w-full h-full cursor-pointer group" style={{ perspective: '1200px' }} onClick={(e) => { e.stopPropagation(); setZoomedCard(zoomedCard === node.id ? null : node.id); }}>
                      ...
                      >
                        ...
                        <img src={`https://flagcdn.com/${node.code}.svg`} alt={node.name} className="w-10 h-[28px] object-cover rounded-[4px] shadow-md border-2 border-white ring-1 ring-black/10 relative z-10" />
                        <span className={`text-sm font-bold mt-2 uppercase tracking-wider px-2 py-0.5 rounded shadow-md transition-colors relative z-10 ${zoomedCard === node.id ? 'bg-[#c91c1c] text-white' : 'bg-white/90 text-[#15325b]'}`}>
                          {node.name}
                        </span>
                      </motion.div>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 w-full relative">
          ...
          >
            ...
            <p className="text-xs text-gray-500 font-medium tracking-[0.2em] uppercase mt-1">LTD • CRO • EU MARKET</p>
          </motion.div>
        </div>
